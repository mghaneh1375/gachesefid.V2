    ...
    <center class="row alaki" style="margin-top: 50px">
        ...
        <table>
            ...
            @foreach($quizes as $quiz)
                <tr>
                    <td><center>{{$quiz->quiz->name}}</center></td>
                    <td><center>{{$quiz->quiz->startDate}}</center></td>
                    @if($quiz->mode == "system")
                        <td></td>
                    @else
                        <td><center>{{$quiz->quiz->endDate}}</center></td>
                    @endif

                    <td><center>{{$quiz->quiz->startTime}}</center></td>

                    @if($quiz->mode == "system")
                        <td></td>
                    @else
                        <td><center>{{$quiz->quiz->endTime}}</center></td>
                    @endif

                    <td><center>{{$quiz->quiz->timeLen}}</center></td>
                    ...
                    @else
                        @if($quiz->quizEntry == 1)
                            <td><button id="btn_{{$quiz->quiz->id}}" onclick="showConfirmationPane('{{$quiz->quiz->id}}')" data-val="{{route('doRegularQuiz', ['quizId' => $quiz->quiz->id])}}" class="btn btn-success">ورود به آزمون</button></td>
                        @elseif($quiz->quizEntry == -2)
                            <td><button onclick="document.location.href = '{{route('showQuizWithOutTime', ['quizId' => $quiz->quiz->id, 'quizMode' => $quiz->quizMode])}}'" class="btn btn-primary">مرور آزمون</button></td>
                        @elseif($quiz->quizEntry == -1)
