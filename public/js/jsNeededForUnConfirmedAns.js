...
$(document).ready(function () {
    currPage = 1;
    fetchUnConfirmedAnses();
});

function fetchUnConfirmedAnses() {
    $.ajax({
        type: 'post',
        url: getUnConfirmedAnsesDir,
        data: {
            'page': currPage
        },
...
function changeSort(val) {
    ...
    if(val == 0) {
        ...
        currPage = 1;
        kindSort = 2;
        fetchConfirmedAndUnConfirmedAnses();
    }

}

function fetchConfirmedAndUnConfirmedAnses() {
    $.ajax({
        type: 'post',
        url: getConfirmedAndUnConfirmedAnses,
        data: {
            'page': currPage
        },
        success: function (response) {
...
function fetchConfirmedAnses() {
    $.ajax({
        type: 'post',
        url: getConfirmedAnsesDir,
        data: {
            'page': currPage
        },
        success: function (response) {
...
function showQuestions(arr) {
    ...
    for(i = 0; i < arr.length; i++) {
        ...
        newElement += "<center><h1 style='margin-top: 10px; border-bottom: 2px solid black'>سوال پرسیده شده</h1></center>";
        newElement += "<textarea class='hidden' id='q_desc_" + arr[i].id + "'>" + arr[i].question + "</textarea>";
        newElement += "<div id='mirrorDescQ_" + arr[i].id + "'></div>";
        newElement += "<center><h1 style='margin-top: 10px; border-bottom: 2px solid black'>پاسخ داده شده شده</h1></center>";
        ...
        if(arr[i].status == 0)
            newElement += "<label class='switch'><input onchange='changeAnsStatus(\"" + arr[i].id + "\")' id='confirm_" + arr[i].id + "' type='checkbox'><span class='slider round'></span></label>";
        else
            newElement += "<label class='switch'><input onchange='changeAnsStatus(\"" + arr[i].id + "\")' id='confirm_" + arr[i].id + "' type='checkbox' checked><span class='slider round'></span></label>";

        newElement += "</center></div>";
        $("#questionsContainer").append(newElement);

        srcIds[srcIds.length] = "desc_" + arr[i].id;
        srcIds[srcIds.length] = "q_desc_" + arr[i].id;
        destIds[destIds.length] = "mirrorDesc_" + arr[i].id;
        destIds[destIds.length] = "mirrorDescQ_" + arr[i].id;
    }
