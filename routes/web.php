...
Route::group(array('middleware' => ['nothing', 'auth', 'adminLevel']), function () {
	...
	Route::get('createOffCode/{type}/{count}/{expireTime}/{amount}', function ($type, $count, $expireTime, $amount) {
		...
		for($i = 0; $i < $count; $i++) {
			...
			$tmp->amount = $amount;
			$code = "gach" . random_int(10000, 99999);
			while (\App\models\OffCode::whereCode($code)->count() > 0)
				$code = "gach" . random_int(10000, 99999);
			$tmp->code = $code;
...
Route::group(array('middleware' => ['nothing', 'auth', 'phone']), function () {
	...
	Route::post('multiPaymentPostQuiz/{mode}/{pack}/{qId1}/{qId2}/{qId3?}/{qId4?}/{qId5?}/{qId6?}/{qId7?}/{qId8?}/{qId9?}/{qId10?}/{qId11?}/{qId12?}/{qId13?}/{qId14?}/{qId15?}/{qId16?}/{qId17?}/{qId18?}/{qId19?}', array('as' => 'multiPaymentPostQuiz', 'uses' => 'QuizController@multiPaymentPostQuiz'));
